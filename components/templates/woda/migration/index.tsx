    ...
    <>
      <Paper p=".8in" className={classes.root} {...configPageProps}>
        ...
        <Divider
          ...
          mb="xl"
        />
        <Text {...propsTitle} lh="22px" ta="center" td="underline" fw={600}>
          MIGRATION CERTIFICATE
          <br /> TO WHOM IY MAY CONCERN
        </Text>
        ...
        >
          ...
          <b>{form.values?.applicant_permanent_address}</b> has migrated from{" "}
          <b>{form.values?.initial_address}</b> to{" "}
          <b>{form.values?.applicant_permanent_address}</b> on{" "}
          {new Date(form.values?.migration_date).getDate()}
          <span
            style={{
              verticalAlign: "super",
              fontSize: 8,
          ...
          >
            {getDaySuffix(new Date(form.values?.migration_date).getDate())}
          </span>{" "}
          {monthNames[new Date(form.values?.migration_date).getMonth()]},{" "}
          {new Date(form.values?.migration_date).getFullYear()} A.D.{" "}
          {form.values?.signature_migration_alongwith}.
        </Text>
        ...
        <Text
          ...
          size="15.4px"
        >
          Thus, the permanent address of{" "}
          <b>
            {form.values?.applicant_honorific} {form.values?.applicant_name}
          </b>{" "}
          {form.values?.signature_migration_alongwith} is now changed into{" "}
          <b>{form.values?.applicant_permanent_address}</b>.
        </Text>
