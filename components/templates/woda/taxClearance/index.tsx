...
export function TemplateTaxClearance() {
  ...
  const calculateFiscalTotals = () => {
    ...
    for (const occupation of form.values?.occupations) {
      sum[0] += occupation.income1;
      sum[1] += occupation.income2;
      sum[2] += occupation.income3;
    }
    return sum;
  };
            ...
            {form.values?.occupations.map((occupation: any, index: number) => {
              return (
                <React.Fragment key={index}>
                  {" "}
                  {occupation.name}
                  {index <= form.values?.occupations.length - 3
                    ? ","
                    : index == form.values?.occupations.length - 2
                    ? " and "
                    : ""}
                </React.Fragment>
              );
          ...
          <tbody>
            ...
            <tr>
              ...
              {fiscalTotals.map((total, index) => (
                <td key={index} style={{ textAlign: "center" }}>
                  <Text fw={600} lh=".2in">
                    {(total * (form.values?.tax / 100)).toLocaleString(
                      undefined,
                      {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
