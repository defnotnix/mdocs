...
//styles
import classes from "./surname.module.css";
//pageprops
...
export function TemplateSurname() {
  // * DEFINITION
    ...
    <>
      <Paper p=".8in" className={classes.root} {...configPageProps}>
        ...
        >
          ...
          <b>{form.values?.applicant_permanent_address}</b> is dissimilar to{" "}
          {form.values?.applicant_surname_reference}{" "}
          <b>{form.values?.applicant_surname}</b> and{" "}
          <b>{form.values?.applicant_parents_surname}</b> respectively. In
          Nepal, <b>{form.values?.applicant_surname}</b> can be written as{" "}
          <b>{form.values?.applicant_parents_surname}</b> legally. All mentioned
          surnames are identical and belong to the same family members.
        </Text>
